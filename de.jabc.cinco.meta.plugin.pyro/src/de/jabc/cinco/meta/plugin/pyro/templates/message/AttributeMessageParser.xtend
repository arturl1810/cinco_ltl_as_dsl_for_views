...
class AttributeMessageParser extends Templateable{
	...
	'''
		...
		public class AttributeMessageParser {
		    ...
		    public static JSONObject attributeElement(String jsonString,C«tc.graphModel.name.toFirstUpper» c«tc.graphModel.name.toFirstUpper»){
            	...
            	«FOR int index : 0..< ModelParser.getNoUserDefinedAttributtes(tc.graphModel.attributes,tc.enums).size »
					JSONObject «ModelParser.getNoUserDefinedAttributtes(tc.graphModel.attributes,tc.enums).get(index).name.toFirstLower» = (JSONObject) element.get(«index»);
					«this.createAttribute(ModelParser.getNoUserDefinedAttributtes(tc.graphModel.attributes,tc.enums).get(index),tc.graphModel.name,tc.enums,tc.graphModel)»
				«ENDFOR»
		        ...
		        «FOR StyledEdge sn:tc.edges»
					...
					«FOR int index : 0..<ModelParser.getNoUserDefinedAttributtes(sn.modelElement.attributes,tc.enums).size »
						JSONObject «ModelParser.getNoUserDefinedAttributtes(sn.modelElement.attributes,tc.enums).get(index).name.toFirstLower» = (JSONObject) element.get(«index»);
						«this.createAttribute(ModelParser.getNoUserDefinedAttributtes(sn.modelElement.attributes,tc.enums).get(index),sn.modelElement.name,tc.enums,tc.graphModel)»
					«ENDFOR»
	...
	def createAttribute(Attribute attribute,String modelName,List<Type> enums,GraphModel graphModel)
	'''
	...
	def createListAttribute(Attribute attribute, String name,List<Type> enums)
	'''
	...
	def createPrimativeAttribute(Attribute attribute, String string, List<Type> enums)
	'''
		// «attribute.name.toFirstUpper»
		«this.createPrimativeAttributeValues(attribute,attribute.name,string,enums)»
	'''
	
	def createPrimativeAttributeValues(Attribute attribute,String attrName, String string, List<Type> enums)
	'''
	...
	def createPrimativeAttributeValuesList(Attribute attribute,String attrName, String string, List<Type> enums)
	'''
