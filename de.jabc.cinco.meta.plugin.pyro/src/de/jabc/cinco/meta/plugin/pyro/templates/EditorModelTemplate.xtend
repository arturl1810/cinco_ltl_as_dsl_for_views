	...
	'''
		...
		«FOR StyledNode node: tc.nodes»
			«IF node.modelElement instanceof Node»
				«createNode(node,tc.enums)»
			«ELSE»
				«createContainer(node,tc.enums)»
			«ENDIF»
		«ENDFOR»
		/*
		 -------------------------------------------------------
...
'''
	...
	def createNode(StyledNode styledNode,List<Type> enums)
	'''
		/**
	...
	def createContainer(StyledNode styledNode,List<Type> enums)
	'''
			/**
		 * «styledNode.modelElement.name.toFirstUpper»
		 * @type {void|*}
