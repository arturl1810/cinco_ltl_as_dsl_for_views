...
class CollectionExtension {
	 ...
	 */
	static def <T> distinctByKey(Seq<T> seq, (T) => Object keyExtractor) {
	 ...
	 */
	static def <T> duplicatesByKey(Seq<T> seq, (T) => Object keyExtractor) {
		val seen = new HashSet
		seq.filter[!(seen += keyExtractor.apply(it))]
	}
	 ...
	 * @see Seq#partition
	 */
	static def <T> partition(Iterable<T> iterable, (T) => boolean predicate) {
	 ...
	 * @see Seq#duplicate
	 */
	static def <T> duplicate(Iterable<T> iterable) {
	 ...
	 * 		for chaining. 
	 */
	static def <T> matching(Tuple2<Seq<T>, Seq<T>> partition, (Seq<T>) => void block) {
	 ...
	 */
	static def <T> nonMatching(Seq<T> seq, (Seq<T>) => void block) {
		block.apply(seq)
	}
	 ...
	 */
	static def <T> orig(Tuple2<Seq<T>, Seq<T>> dupes, (Seq<T>) => void block) {
	 ...
	 * @param block the lambda to be executed on the remaining elements.
	 */
	static def <T> copy(Seq<T> seq, (Seq<T>) => void block) {
		block.apply(seq)
	}
	 ...
	 */
	static def <K,V, N extends K> Seq<Tuple2<N, V>> filterKeys(Seq<Tuple2<K, V>> mapSeq, Class<N> c) {
		val it = mapSeq.unzip
		v1.ofType(c).zip(v2)
	}
	 ...
	 */
	static def <T, U> associateWithKey(Iterable<U> iterable, (U) => T keyExtractor) {
	 ...
	 */
	static def <T, U> associateWithKey(Seq<U> seq, (U) => T keyExtractor) {
		seq.map[new Tuple2(keyExtractor.apply(it), it)]
	}
	
	static def <T> match(Iterable<T> iterable, (T) => boolean predicate) {
