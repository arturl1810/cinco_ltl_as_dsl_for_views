/**
 * Created by Philip Zweihoff on 07.04.15.
 */
/**
 * Event-Listener
 * Del Key Hit
...
$('.remove-graph').click(function(){
   $('#removeGraphName').text($(this).attr('data-graph-name'));
});
...
$( "#paper-link-out" ).focusout(function() {
    ...
    //TODO
    console.log('[AJAX] Send Attribute changed');
});
...
graph.on('add', function(cell) {
    ...
    styleAllEdges();
    styleElements(false);
    sendChangeAction(cell,cell.attributes.cinco_type,'create');
});
...
$('#elementChange')
    .on('cellResize', function(event,cell){
        sendChangeAction(cell,cell.attributes.cinco_type,'resize');
    })
    .on('cellRotate', function(event,cell){
        sendChangeAction(cell,cell.attributes.cinco_type,'rotate');
    });
...
graph.on('remove', function(cell) {
    ...
    sendChangeAction(cell,cell.attributes.cinco_type,'delete');
});
...
$(function() {
    $( "#accordion").find("li")
        .mousedown(function() {
            isDragging = true;
            $('.papyrus-container').css('cursor', 'move');
            disableSelection($('.papyrus-container'));
    ...
    $("#paper").mouseup(function(e) {
        if(isDragging){
            isDragging = false;
            $('.papyrus-container').css('cursor', 'auto');
    ...
    $(".papyrus-container").mouseup(function(e) {
       if(isDragging){
           isDragging = false;
           $('.papyrus-container').css('cursor', 'auto');
       }
